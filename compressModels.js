...
folders.forEach(folder => {
  ...
  fs.readdirSync(folderPath).forEach(file => {
    if (path.extname(file) === '.glb' && !file.startsWith('compressed_')) {
      ...
      const command = `gltf-pipeline -i ${inputPath} -o ${outputPath} -d --draco.quantizePositionBits 8 --draco.quantizeTexcoordBits 8 --draco.quantizeNormalBits 6 --draco.quantizeGenericBits 6`;      console.log(`Compressing: ${file} in folder ${folder}`);
